<?php
/*********************************************************************************
 * The contents of this file are subject to the SugarCRM Public License Version 1.1.2
 * ("License"); You may not use this file except in compliance with the
 * License. You may obtain a copy of the License at http://www.sugarcrm.com/SPL
 * Software distributed under the License is distributed on an  "AS IS"  basis,
 * WITHOUT WARRANTY OF ANY KIND, either express or implied. See the License for
 * the specific language governing rights and limitations under the License.
 * The Original Code is:  SugarCRM Open Source
 * The Initial Developer of the Original Code is SugarCRM, Inc.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.;
 * All Rights Reserved.
 * Contributor(s): ______________________________________.
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/vtiger_crm/sugarcrm/modules/Potentials/language/en_us.lang.php,v 1.18 2005/04/28 14:05:53 ray Exp $
 * Description:  Defines the English language pack for the Account module.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
 ********************************************************************************/

$mod_strings = Array(
'LBL_MODULE_NAME'=>'הזדמנויות',
'LBL_MODULE_TITLE'=>'הזדמנויות: בית',
'LBL_SEARCH_FORM_TITLE'=>'חיפוש הזדמנות',
'LBL_LIST_FORM_TITLE'=>'רשימת הזדמנויות',
'LBL_OPPORTUNITY_NAME'=>'שם ההזדמנות:',
'LBL_OPPORTUNITY'=>'הזדמנות:',
'LBL_NAME'=>'שם ההזדמנות',
'LBL_INVITEE'=>'אנשי קשר',
'LBL_LIST_OPPORTUNITY_NAME'=>'הזדמנות',
'LBL_LIST_ACCOUNT_NAME'=>'שם חשבון',
'LBL_PRODUCT_NAME'=>'שם מוצר',
'LBL_LIST_AMOUNT'=>'כמות',
'LBL_LIST_DATE_CLOSED'=>'תאריך סגירה צפוי',
'LBL_LIST_SALES_STAGE'=>'שלב המכירה',

'LBL_OPPORTUNITY_NAME'=>'שם ההזדמנות:',
'LBL_ACCOUNT_NAME'=>'שם חשבון:',
'LBL_AMOUNT'=>'כמות:',
'LBL_DATE_CLOSED'=>'תאריך סגירה צפוי:',
'LBL_TYPE'=>'סוג:',
'LBL_NEXT_STEP'=>'שלב הבא:',
'LBL_LEAD_SOURCE'=>'מקור הקשר:',
'LBL_SALES_STAGE'=>'שלב המכירה:',
'LBL_PROBABILITY'=>'סבירות (%):',
'LBL_DESCRIPTION'=>'תיאור:',
'LBL_OPPORTUNITY_INFORMATION'=>'מידע ההזדמנות:',
'LBL_DESCRIPTION_INFORMATION'=>'תיאור ההזדמנות:',
'LBL_CUSTOM_INFORMATION'=>'מידע מותאם אישית',

'LBL_NEW_FORM_TITLE'=>'הזדמנות חדשה',

'ERR_DELETE_RECORD'=>"A record number must be specified to delete the opportunity.",
'LBL_TOP_OPPORTUNITIES'=>"ההזדמנויות הפתוחות הטובות ביותר שלי",

'NTC_REMOVE_OPP_CONFIRMATION'=>'Are you sure you want to remove this contact from this opportunity?',

'NTC_NONE_SCHEDULED'=>'כלום לא נקבע.',

'LBL_SELECT_OPPOPTUNITY'=>'בחירת הזדמנות',
'LBL_GENERAL_INFORMATION'=>'מידע כללי',



//for v4 release added
'LBL_NEW_POTENTIAL'=>'הזדמנות חדשה',
'LBL_POTENTIAL_TITLE'=>'הזדמנויות',

'LBL_NEW_TASK'=>'משימה חדשה',
'LBL_TASK_TITLE'=>'משימות',
'LBL_NEW_CALL'=>'שיחת טלפון חדשה',
'LBL_CALL_TITLE'=>'שיחות טלפון',
'LBL_NEW_MEETING'=>'פגישה חדשה',
'LBL_MEETING_TITLE'=>'פגישות',
'LBL_NEW_EMAIL'=>'דוא&quot;ל חדש',
'LBL_EMAIL_TITLE'=>'דוא&quot;ל',
'LBL_NEW_NOTE'=>'הערה חדשה',
'LBL_NOTE_TITLE'=>'הערות',

'LBL_NEW_ATTACHMENT'=>'מסמך מצורף חדש',
'LBL_ATTACHMENT_TITLE'=>'מסמכים מצורפים',

'LBL_NEW_CONTACT'=>'איש קשר חדש',
'LBL_CONTACT_TITLE'=>'אנשי קשר',

//Added vtiger_fields after RC1 - Release
'LBL_ALL'=>'הכל',
'LBL_WON'=>'הסתיים בזכייה',
'LBL_LOST'=>'הסתיים בהפסד',
'LBL_VALUE_PROPOSITION'=>'הצעת שווי',
'LBL_PROSPECTING'=>'בבדיקה',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'כלי הזדמנויות',
//Added for 4GA
'Potential Name'=>'שם ההזדמנות',
'Amount'=>'כמות',
'Account Name'=>'שם החשבון',
'Currency'=>'מטבע',
'Expected Close Date'=>'תאריך סגירה צפוי',
'Opportunity Type'=>'סוג ההזדמנות',
'Type'=>'סוג',
'Next Step'=>'שלב הבא',
'Lead Source'=>'מקור הקשר',
'Sales Stage'=>'שלב המכירה',
'Assigned To'=>'מצוות אל',
'Probability'=>'סבירות (%)',
'Campaign Source'=>'מקור מסע הפרסום',
'Description'=>'תיאור',
'Created Time'=>'תאריך יצירה',
'Modified Time'=>'תאריך שינוי',
//Added for 4.2 Release -- CustomView
'Potential'=>'הזדמנות',
'Expected Close'=>'תאריך סגירה צפוי',

//Added for Existing Picklist Strings

'Prospecting'=>'בבדיקה',
'Qualification'=>'בתהליכי אישור',
'Needs Analysis'=>'דרוש ניתוח',
'Value Proposition'=>'הצעת שווי',
'Id. Decision Makers'=>'מקבלי החלטות מובילים',
'Perception Analysis'=>'Perception Analysis',
'Proposal/Price Quote'=>'הצעת מחיר',
'Negotiation/Review'=>'משא ומתן / בחינה',
'Closed Won'=>'נסגר בהצלחה',
'Closed Lost'=>'נסגר בכשלון',

'Cold Call'=>'שיחת טלפון',
'Existing Customer'=>'לקוח קיים',
'Self Generated'=>'נוצר בעצמו',
'Employee'=>'עובד',
'Partner'=>'שותף',
'Public Relations'=>'יחסי ציבור',
'Direct Mail'=>'דיוור ישיר',
'Conference'=>'ועידה',
'Trade Show'=>'כנס',
'Web Site'=>'אתר אינטרנט',
'Word of mouth'=>'שמועה מפה לאוזן',
'Other'=>'אחר',

'--None--'=>'--כלום--',
'Existing Business'=>'עסק קיים',
'New Business'=>'עסק חדש',

// Module Sequence Numbering
'Potential No' => 'מספר ההזדמנות',
// END

);

?>
